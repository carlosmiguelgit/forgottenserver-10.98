...
void Game::playerMoveCreature(Player* player, Creature* movingCreature, const Position& movingCreatureOrigPos, Tile* toTile) {
	if (!player->canDoAction()) {
		...
		SchedulerTask* task = createSchedulerTask(delay, [=, this, playerID = player->getID(), movingCreatureID = movingCreature->getID(), toPos = toTile->getPosition()]() {
	...
	if (!movingCreatureOrigPos.isInRange(player->getPosition(), 1, 1, 0)) {
		...
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(RANGE_MOVE_CREATURE_INTERVAL, [=, this, playerID = player->getID(), movingCreatureID = movingCreature->getID(), toPos = toTile->getPosition()] {
					playerMoveCreatureByID(playerID, movingCreatureID, movingCreatureOrigPos, toPos);
			});
	...
	if (!events::player::onMoveCreature(player, movingCreature, movingCreaturePos, toPos)) {
		return;
	}
...
void Game::playerMoveItem(Player* player, const Position& fromPos,
	...
	if (item->isHangable() && toCylinderTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
		...
		if (!playerPos.isInRange(mapToPos, 1, 1, 0)) {
			...
			if (fromPos.x != 0xFFFF && mapFromPos.isInRange(playerPos, 1, 1) && !mapFromPos.isInRange(walkPos, 1, 1, 0)) {
			...
			if (player->getPathTo(walkPos, listDir, 0, 0, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(RANGE_MOVE_ITEM_INTERVAL,[this, playerID = player->getID(), itemPos, spriteId, itemStackPos, toPos, count]() {
...
ReturnValue Game::internalMoveItem(Cylinder* fromCylinder, Cylinder* toCylinder, int32_t index,
	...
	if (actorPlayer && fromPos && toPos) {
		const ReturnValue ret = events::player::onMoveItem(actorPlayer, item, count, *fromPos, *toPos, fromCylinder, toCylinder);
		if (ret != RETURNVALUE_NOERROR) {
			return ret;
		}
	}
	...
	if (ret == RETURNVALUE_NEEDEXCHANGE) {
		...
		if (ret == RETURNVALUE_NOERROR) {
			if (actorPlayer && fromPos && toPos) {
				const ReturnValue eventRet = events::player::onMoveItem(actorPlayer, toItem, toItem->getItemCount(), *toPos, *fromPos, toCylinder, fromCylinder);
			...
			if (toCylinder->queryRemove(*toItem, toItem->getItemCount(), flags, actor) == RETURNVALUE_NOERROR) {
				...
				if (actorPlayer && fromPos && toPos && !toItem->isRemoved()) {
					events::player::onItemMoved(actorPlayer, toItem, toItem->getItemCount(), *toPos, *fromPos, toCylinder, fromCylinder);
				}
	...
	if (actorPlayer && fromPos && toPos) {
		if (updateItem && !updateItem->isRemoved()) {
			events::player::onItemMoved(actorPlayer, updateItem, count, *fromPos, *toPos, fromCylinder, toCylinder);
		} else if (moveItem && !moveItem->isRemoved()) {
			events::player::onItemMoved(actorPlayer, moveItem, count, *fromPos, *toPos, fromCylinder, toCylinder);
		} else if (item && !item->isRemoved()) {
			events::player::onItemMoved(actorPlayer, item, count, *fromPos, *toPos, fromCylinder, toCylinder);
		}
	}
...
void Game::playerEquipItem(uint32_t playerId, uint16_t spriteId) {
	...
	if (slotItem && slotItem->getID() == it.id &&
		...
		internalMoveItem(slotItem->getParent(), player, CONST_SLOT_WHEREEVER, slotItem, slotItem->getItemCount(), nullptr, 0, player, nullptr, &fromPos, &toPos);
	} else if (equipItem) {
		internalMoveItem(equipItem->getParent(), player, slot, equipItem, equipItem->getItemCount(), nullptr, 0, player, nullptr, &fromPos, &toPos);
	}
}
...
void Game::playerUseItemEx(uint32_t playerId, const Position& fromPos, uint8_t fromStackPos, uint16_t fromSpriteId,
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (fromPos.x != 0xFFFF && toPos.x != 0xFFFF && fromPos.isInRange(player->getPosition(), 1, 1, 0) && !fromPos.isInRange(toPos, 1, 1, 0)) {
				Item* moveItem = nullptr;
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(RANGE_USE_ITEM_INTERVAL, [=, this]() {
...
void Game::playerUseWithCreature(uint32_t playerId, const Position& fromPos, uint8_t fromStackPos, uint32_t creatureId, uint16_t spriteId) {
	...
	if (!creature->getPosition().isInRange(player->getPosition(), Map::maxClientViewportX - 1, Map::maxClientViewportY - 1, 0)) {
		return;
	}
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (fromPos.x != 0xFFFF && fromPos.isInRange(player->getPosition(), 1, 1, 0) && !fromPos.isInRange(toPos, 1, 1, 0)) {
				Item* moveItem = nullptr;
				ret = internalMoveItem(item->getParent(), player, INDEX_WHEREEVER, item, item->getItemCount(), &moveItem, 0, player, nullptr, &fromPos, &toPos);
				if (ret != RETURNVALUE_NOERROR) {
					player->sendCancelMessage(ret);
...
void Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid) {
	...
	if (!parentContainer) {
		...
		if (!events::player::onBrowseField(player, tile->getPosition())) {
			return;
		}
		...
		if (it == browseFields.end()) {
			...
			g_scheduler.addEvent(createSchedulerTask(30000, [this, position = tile->getPosition()]() {
				decreaseBrowseFieldRef(position);
			}));
		} else {
...
void Game::playerRotateItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId) {
	...
	if (pos.x != 0xFFFF && !pos.isInRange(player->getPosition(), 1, 1, 0)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(RANGE_ROTATE_ITEM_INTERVAL, [=, this]() {
	...
	events::player::onRotateItem(player, item);
}
...
void Game::playerBrowseField(uint32_t playerId, const Position& pos) {
	...
	if (!playerPos.isInRange(pos, 1, 1)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(RANGE_BROWSE_FIELD_INTERVAL, [=, this]() {
	...
	if (!events::player::onBrowseField(player, pos)) {
		return;
	}
...
void Game::playerWrapItem(uint32_t playerId, const Position& position, uint8_t stackPos, const uint16_t spriteId) {
	...
	if (position.x != 0xFFFF && !position.isInRange(player->getPosition(), 1, 1, 0)) {
		...
		if (player->getPathTo(position, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(RANGE_WRAP_ITEM_INTERVAL, [=, this]() {
				playerWrapItem(playerId, position, stackPos, spriteId);
			});
			player->setNextWalkActionTask(task);
		} else {
			player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
	...
	events::player::onWrapItem(player, item);
}
...
void Game::playerRequestTrade(uint32_t playerId, const Position& pos, uint8_t stackPos,
	...
	if (!events::player::onTradeRequest(player, tradePartner, tradeItem)) {
		return;
	}
...
void Game::playerAcceptTrade(uint32_t playerId) {
	...
	if (tradePartner->getTradeState() == TRADE_ACCEPT) {
		...
		if (!events::player::onTradeAccept(player, tradePartner, playerTradeItem, partnerTradeItem)) {
			internalCloseTrade(player, false);
			return;
		}
		...
		events::player::onTradeCompleted(player, tradePartner, playerTradeItem, partnerTradeItem, isSuccess);
...
void Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, uint8_t index) {
	...
	int32_t lookDistance = std::max(playerPosition.getDistanceX(tradeItemPosition), playerPosition.getDistanceY(tradeItemPosition));
	if (index == 0) {
		events::player::onLookInTrade(player, tradePartner, tradeItem, lookDistance);
		return;
	}
	...
	while (i < containers.size()) {
		...
		for (Item* item : container->getItemList()) {
			...
			if (--index == 0) {
				events::player::onLookInTrade(player, tradePartner, item, lookDistance);
				return;
			}
		}
	}
...
void Game::playerLookInShop(uint32_t playerId, uint16_t spriteId, uint8_t count) {
	...
	events::player::onLookInShop(player, &it, subType, description);
}
...
void Game::playerLookAt(uint32_t playerId, const Position& pos, uint8_t stackPos) {
	...
	events::player::onLook(player, pos, thing, stackPos, lookDistance);
}
...
void Game::playerLookInBattleList(uint32_t playerId, uint32_t creatureId) {
	...
	events::player::onLookInBattleList(player, creature, lookDistance);
}
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId) {
	...
	g_dispatcher.addTask([this, id = player->getID()]() {
		updateCreatureWalk(id);
	});
}
...
void Game::playerTurn(uint32_t playerId, Direction dir) {
	...
	if (!events::player::onTurn(player, dir)) {
		return;
	}
	...
	if (!echo) {
		for (Creature* spectator : spectators) {
			...
			if (creature != spectator) {
				events::creature::onHear(spectator, creature, text, type);
			}
		}
	}
	return true;
...
void Game::checkCreatures(size_t index) {
	g_scheduler.addEvent(createSchedulerTask(EVENT_CHECK_CREATURE_INTERVAL, [=, this]() {
...
void Game::internalCreatureChangeOutfit(Creature* creature, const Outfit_t& outfit) {
	if (!events::creature::onChangeOutfit(creature, outfit)) {
		return;
	}
...
void Game::playerInviteToParty(uint32_t playerId, uint32_t invitedId) {
	...
	if (!events::party::onInvite(party, invitedPlayer)) {
		if (party->empty()) {
...
void Game::playerReportRuleViolation(uint32_t playerId, const std::string& targetName, uint8_t reportType, uint8_t reportReason, const std::string& comment, const std::string& translation) {
	...
	events::player::onReportRuleViolation(player, targetName, reportType, reportReason, comment, translation);
}
...
void Game::playerReportBug(uint32_t playerId, const std::string& message, const Position& position, uint8_t category) {
	...
	events::player::onReportBug(player, message, position, category);
}
...
void Game::playerDebugAssert(uint32_t playerId, const std::string& assertLine, const std::string& date, const std::string& description, const std::string& comment) {
	...
	if (file) {
		fprintf(file, "----- %s - %s (%s) -----\n", formatDate(time(nullptr)).c_str(), player->getName().c_str(), player->getIP().to_string().c_str());
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		if (it.stackable) {
			...
			while (tmpAmount > 0) {
				...
				if (internalAddItem(player->getInbox().get(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
					delete item;
					break;
				}
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		if (it.stackable) {
			...
			while (tmpAmount > 0) {
				...
				Item* item = Item::CreateItem(it.id, stackCount);
				if (internalAddItem(buyerPlayer->getInbox().get(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
					delete item;
					break;
				}
...
bool Game::reload(ReloadTypes_t reloadType) {
	switch (reloadType) {
		...
		case RELOAD_TYPE_CONFIG: return ConfigManager::reload();
		case RELOAD_TYPE_CREATURESCRIPTS: {
			g_creatureEvents->reload();
			g_creatureEvents->removeInvalidEvents();
			return true;
		}
		case RELOAD_TYPE_EVENTS: return events::reload();
