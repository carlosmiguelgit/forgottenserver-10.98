	...
	for (const auto& it : doorSet) {
		if (owner == 0) {
			it->setSpecialDescription(fmt::format(
				"Nobody owns this house.{:s}",
				getBoolean(ConfigManager::HOUSE_DOOR_SHOW_PRICE) &&
				(housePrice != -1)
					? fmt::format(
						" It costs {:d} gold coins.",
						houseTiles.size() * housePrice
					)
					: ""
			));
		} else {
...
bool House::transferToDepot(Player* player) const {
	...
	for (Item* item : moveItemList) {
		g_game.internalMoveItem(item->getParent(), player->getInbox().get(), INDEX_WHEREEVER, item, item->getItemCount(), nullptr, FLAG_NOLIMIT);
	}
	return true;
}
